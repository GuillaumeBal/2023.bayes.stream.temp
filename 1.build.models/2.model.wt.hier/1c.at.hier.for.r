...
for(sm in 1:n.6m.windows.hist){
  ...
  alpha.at[sm] ~ dnorm(mu.alpha.at, tau.alpha.at) 
  beta.at[sm] ~ dlnorm(log(mu.beta.at) - pow(sd.beta.at, 2) / 2, tau.beta.at)
...
mu.alpha.at ~ dnorm(p.mean.alpha.at, .1)
#tau.alpha.at ~ dgamma(0.001, 0.001)
#sd.alpha.at <- sqrt(1 / tau.alpha.at)
tau.alpha.at <- 1 / pow(sd.beta.at, 2)
sd.alpha.at ~ dgamma(2, 1)

mu.beta.at ~ dlnorm(log(p.mean.beta.at) - .5, 1)
#tau.beta.at ~ dgamma(0.001, 0.001)
#sd.beta.at <- sqrt(1 / tau.beta.at)
tau.beta.at <- 1 / pow(sd.beta.at, 2)
sd.beta.at ~ dgamma(2, 1)

for(sm in (n.6m.windows.hist + 1) : n.6m.windows.for){
  alpha.at[sm] ~ dnorm(p.mean.alpha.at - .5, 1)
